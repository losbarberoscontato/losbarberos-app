...
begin
  ...
  update public.financial_entries set canceled_at=now(), cancellation_reason='Lançamento manual substituído por edição' where id=v_entry.id and organization_id=v_entry.organization_id;
...
create or replace function public.delete_manual_cash_entry(p_settlement_id uuid, p_idempotency_key text)
returns uuid language plpgsql security definer set search_path = public, pg_temp as $$
declare v_source public.financial_settlements%rowtype; v_entry public.financial_entries%rowtype; v_existing public.financial_settlements%rowtype; v_id uuid;
begin
  if nullif(btrim(p_idempotency_key), '') is null then raise exception using errcode='22023', message='idempotency key is required'; end if;
  select s.* into strict v_source from public.financial_settlements s where s.id=p_settlement_id for update;
  select e.* into strict v_entry from public.financial_entries e where e.id=v_source.entry_id for update;
  perform public.require_financial_owner(v_entry.organization_id, 'manual cash entry deletion');
  if v_entry.source <> 'MANUAL' or v_source.kind <> 'SETTLEMENT' then raise exception using errcode='22023', message='only manual cash entries can be deleted'; end if;
  ...
  v_id := public.reverse_financial_settlement(p_settlement_id, v_source.amount_cents, 'Exclusão de lançamento manual', p_idempotency_key||':reversal');
  update public.financial_entries set canceled_at=now(), cancellation_reason='Lançamento manual excluído' where id=v_entry.id and organization_id=v_entry.organization_id;
  return v_id;
end; $$;

revoke all on function public.delete_manual_cash_entry(uuid,text) from public, anon, authenticated, service_role;
grant execute on function public.delete_manual_cash_entry(uuid,text) to authenticated;
