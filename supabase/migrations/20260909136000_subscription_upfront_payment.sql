-- À vista liquida todos os ciclos do plano em um único recebimento.
...
begin
  ...
  select coalesce(sum(amount_cents), 0)::bigint into v_required from public.customer_subscription_cycles where subscription_id=v_sub.id and organization_id=p_organization_id and status in ('OPEN','OVERDUE');
  if not v_sub.upfront_payment then v_required := v_cycle.amount_cents; end if;
  if p_amount_cents <> v_required then raise exception using errcode='22023', message='subscription payment amount does not match payable balance'; end if;
  ...
  values(p_organization_id,'REVENUE','SUBSCRIPTION',case when v_sub.upfront_payment then 'Plano de assinatura à vista · '||v_sub.id else 'Parcela de assinatura · '||v_sub.id end,current_date,current_date,v_cycle.due_on,p_amount_cents,'BRL',p_chart_account_id,p_financial_account_id,'CUSTOMER',v_sub.customer_id,v_cycle.id,auth.uid()) returning id into v_entry;
  ...
  if v_sub.upfront_payment then
    update public.customer_subscription_cycles set status='PAID',paid_at=now(),financial_entry_id=v_entry where subscription_id=v_sub.id and organization_id=p_organization_id and status in ('OPEN','OVERDUE');
  else
