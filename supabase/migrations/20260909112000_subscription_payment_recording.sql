...
begin
  ...
  if p_amount_cents <= 0 or p_amount_cents > v_cycle.amount_cents then raise exception using errcode='22023', message='invalid subscription payment amount'; end if;
  ...
  if p_amount_cents = v_cycle.amount_cents then update public.customer_subscription_cycles set status='PAID',paid_at=now(),financial_entry_id=v_entry where id=v_cycle.id; end if;
  return v_payment;
end $$;
