...
export function metaGraphVersion(): string {
  ...
  if (!/^v\d+\.\d+$/u.test(version)) throw new TypeError("Invalid Meta Graph version");
  return version;
}

export async function metaGraphRequest<T extends MetaGraphObject = MetaGraphObject>(
  path: string,
  init: RequestInit = {},
): Promise<T> {
...
export async function exchangeMetaSignupCode(code: string): Promise<string> {
  ...
  const response = await providerFetch<{ access_token?: string }>(url.toString(), { method: "GET" });
...
export function metaAuthorizationUrl(state: string): string {
  ...
  url.searchParams.set("config_id", requiredEnv("WHATSAPP_EMBEDDED_SIGNUP_CONFIG_ID"));
  url.searchParams.set("response_type", "code");
  url.searchParams.set("state", state);
  url.searchParams.set("redirect_uri", requiredEnv("WHATSAPP_META_REDIRECT_URI"));
