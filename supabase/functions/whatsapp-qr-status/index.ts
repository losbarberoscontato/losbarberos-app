...
type ConnectionStateResponse = { instance?: { state?: unknown }; state?: unknown };
...
Deno.serve((request) => {
  ...
  try {
    ...
    for (let attempt = 0; state === "connecting" && attempt < 15; attempt += 1) {
    ...
    if (error instanceof IntegrationError && error.code === "PROVIDER_STATE_MISSING") throw error;
    await rpc("record_whatsapp_qr_health", {
      p_gateway_instance_id: target.gateway_instance_id,
      p_provider_state: null,
