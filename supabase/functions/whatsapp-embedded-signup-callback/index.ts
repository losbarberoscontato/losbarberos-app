...
function redirectWith(path: string, status: string): Response {
  const url = new URL(
    safeReturnPath(path, "/gestor/configuracoes/whatsapp"),
    appOrigin(),
  );
...
Deno.serve((request) =>
  endpoint(request, async () => {
    ...
    if (!state || state.length > 512) {
      throw new IntegrationError(400, "INVALID_CONNECTION_STATE");
    }
    const context = await rpc<StateContext | null>(
      "consume_whatsapp_connection_state",
      {
    ...
    if (!code || code.length > 4096) {
      return redirectWith(context.return_path, "canceled");
    }

    const accessToken = await exchangeMetaSignupCode(code);
    const business = await metaGraphRequest<
      {
        data?: Array<
          {
            id?: string;
    ...
    const account = business.data?.find((item) =>
      item.owned_whatsapp_business_accounts?.data?.[0]
    );
    const waba = account?.owned_whatsapp_business_accounts?.data?.[0];
    const phoneNumberId = waba?.phone_numbers?.data?.[0]?.id;
    if (
      !account?.id || !waba?.id || !phoneNumberId
    ) {
      throw new IntegrationError(502, "META_BUSINESS_NOT_FOUND", true);
    }
