...
describe("barber app migration", () => {
  ...
  it("keeps reconciliation immutable, tenant-scoped and linked to original cash activity", () => {
    expect(reconciliationMigration).toContain("create table public.barber_cash_reconciliations");
    expect(reconciliationMigration).toContain("unique (organization_id, cash_session_id)");
    expect(reconciliationMigration).toContain("before update or delete");
    expect(reconciliationMigration).toContain("p_account_breakdown jsonb");
    expect(reconciliationMigration).toContain("conciliation_at");
