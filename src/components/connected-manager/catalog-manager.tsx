...
export function CatalogManager({ organizationId, services, packages, packageItems }: Props) {
  ...
  function editPackage(item: PackageRecord) {
    setPackageForm(item);
    setSelectedServices(packageItems.filter((link) => link.package_id === item.id).map((link) => link.service_id));
  }
    ...
    <div className={styles.grid}>
      <Panel title="Serviços" description={`${services.filter((item) => item.active).length} ativos`} className={styles.span6} action={<button className={styles.button} type="button" onClick={() => setServiceForm("new")}>Novo serviço</button>}>
        ...
        {services.length === 0 ? <EmptyState title="Sem serviços">Cadastre o primeiro procedimento.</EmptyState> : <div className={styles.list}>{services.map((service) => <article className={styles.row} key={service.id}>
          <span className={styles.rowTitle}><strong>{service.name}</strong><small>{service.description ?? "Sem descrição"}</small></span>
      ...
      <Panel title="Pacotes" description="Uso em uma única visita" className={styles.span6} action={<button className={styles.button} type="button" disabled={!services.some((item) => item.active)} onClick={() => { setPackageForm("new"); setSelectedServices([]); }}>Novo pacote</button>}>
        ...
        {packages.length === 0 ? <EmptyState title="Sem pacotes">Crie combinações depois de cadastrar serviços.</EmptyState> : <div className={styles.list}>{packages.map((item) => <article className={styles.row} key={item.id}>
          <span className={styles.rowTitle}><strong>{item.name}</strong><small>{packageItems.filter((link) => link.package_id === item.id).map((link) => serviceById.get(link.service_id)?.name).filter(Boolean).join(" + ") || "Sem itens"}</small></span>
          <strong>{formatCents(item.price_cents)}</strong><span>{packageItems.filter((link) => link.package_id === item.id).length} itens</span><StatusChip active={item.active} />
