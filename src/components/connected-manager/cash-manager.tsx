...
export function CashManager(props: CashManagerProps) {
  ...
  const dailyMovement = cashSettlementMovements.filter(({ settlement }) => movementDate(settlement.settled_on, timezone) === today(timezone)).reduce((total, { entry, settlement }) => total + settlementSignedCents(entry.kind, settlement), 0) + props.appointmentActivity.filter((item) => movementDate(item.occurred_at, timezone) === today(timezone)).reduce((total, item) => total + item.signed_cents, 0) - (props.commissionSettlements ?? []).filter((item) => movementDate(item.paid_on, timezone) === today(timezone)).reduce((total, item) => total + item.amount_cents, 0);
  const periodMovement = visibleSettlements.reduce((total, { entry, settlement }) => total + settlementSignedCents(entry.kind, settlement), 0) + visibleActivity.reduce((total, item) => total + item.signed_cents, 0) - visibleCommissionSettlements.reduce((total, item) => total + item.amount_cents, 0);
  ...
  return <div className={styles.stack}>
    ...
    {(props.section === "cash" || props.section === "payables" || props.section === "receivables") && <>
      ...
      <CashList cashOnly={props.section === "cash"} timezone={timezone} entries={visibleEntries} settlements={props.section === "cash" ? visibleSettlements : []} commissionSettlements={visibleCommissionSettlements} activity={visibleActivity} receivables={visibleAppointmentReceivables} accountById={accountById} supplierById={supplierById} customerById={customerById} barberNames={props.barberNames ?? {}} onEdit={setEntryEditor} onEditSettlement={setCashEdit} onSettle={setSettlementEntry} onReceive={setAppointmentReceipt} onCancel={async (entry) => {
...
function CashList({ cashOnly, timezone, entries, settlements, commissionSettlements, activity, receivables, accountById, supplierById, customerById, barberNames, onEdit, onEditSettlement, onSettle, onReceive, onCancel, onTransfer, onReverseAppointment }: { cashOnly: boolean; timezone: string; entries: FinancialEntryRecord[]; settlements: CashSettlementMovement[]; commissionSettlements: CommissionPayoutSettlementRecord[]; activity: AppointmentCashActivityRecord[]; receivables: AppointmentReceivableRecord[]; accountById: Map<string, FinancialAccountRecord>; supplierById: Map<string, SupplierRecord>; customerById: Map<string, Pick<CustomerRecord, "id" | "organization_id" | "full_name" | "active">>; barberNames: Record<string, string>; onEdit: (entry: FinancialEntryRecord) => void; onEditSettlement: (movement: CashSettlementMovement) => void; onSettle: (entry: FinancialEntryRecord) => void; onReceive: (entry: AppointmentReceivableRecord) => void; onCancel: (entry: FinancialEntryRecord) => void; onTransfer: () => void; onReverseAppointment: (entry: AppointmentCashActivityRecord) => void }) {
  return <Panel title="Movimentações" description={cashOnly ? undefined : "Registros de agendamento são vinculados ao ledger existente e não podem ser editados aqui."} action={<button className={`${styles.button} ${styles.buttonSoft}`} type="button" onClick={onTransfer}><ArrowLeftRight size={15} /> Transferir</button>}>
    {!entries.length && !settlements.length && !commissionSettlements.length && !activity.length && !receivables.length ? <EmptyState title="Sem movimentações">Registre um recebimento ou pagamento para acompanhar o caixa.</EmptyState> : <div className={`${styles.cashTable} ${cashOnly ? styles.cashTableOnly : ""}`} role="table" aria-label="Movimentações financeiras">
      ...
      {commissionSettlements.map((settlement) => <article key={settlement.id} className={styles.cashRow} role="row"><span className={styles.rowTitle} role="cell"><strong className={styles.cashCounterparty}>{barberNames[settlement.barber_id] ?? "Profissional"}</strong><small className={styles.cashDescription}>Pagamento de comissão{settlement.tags ? ` · ${settlement.tags}` : ""}</small></span><span role="cell">{formatMovementDate(settlement.paid_on, timezone)}</span><strong role="cell">−{formatCents(settlement.amount_cents)}</strong><span role="cell">{accountById.get(settlement.financial_account_id)?.name ?? "Conta não encontrada"}</span><span className={styles.rowActions} role="cell" /> </article>)}
