...
function EntryDialog({ entry, defaultKind, settleImmediately = false, replacementSettlementId, organizationId, accounts, suppliers, chartAccounts, costCenters, tags, customers, entryTags, demoMode, onClose, onSaved, setMessage }: Omit<CashManagerProps, "section" | "billingStatus" | "balances" | "entries" | "settlements" | "appointmentActivity" | "mappings" | "appointmentReceivables"> & { entry: FinancialEntryRecord | null; defaultKind: EntryKind; settleImmediately?: boolean; replacementSettlementId?: string; onClose: () => void; onSaved: () => void; setMessage: (value: string) => void }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    const saved = await runMutation(setMessage, async () => {
      if (replacementSettlementId) await assertResult(await connectedClient().rpc("edit_manual_cash_entry_with_reversal", { p_settlement_id: replacementSettlementId, ...params, p_financial_account_id: safeText(data.get("account_id")), p_payment_method: safeText(data.get("payment_method")), p_reference: safeText(data.get("reference")) || null, p_idempotency_key: `manager:cash-edit:${replacementSettlementId}:${crypto.randomUUID()}` }));
    ...
    }, replacementSettlementId ? "Lançamento revertido e substituído no Caixa." : entry ? "Lançamento aberto atualizado." : isSeries ? "Série de despesas criada." : settleImmediately ? "Lançamento liquidado e registrado no Caixa." : "Lançamento criado.");
    if (saved) onSaved();
  }
  return <Dialog title={entry ? "Editar lançamento" : "Novo lançamento"} onClose={onClose} wide><form className={styles.form} onSubmit={submit}>
    ...
    {!entry && kind === "EXPENSE" && !settleImmediately && <Field label="Tipo de despesa"><select value={expenseType} onChange={(event) => setExpenseType(event.target.value as "SINGLE" | "RECURRING" | "INSTALLMENT")}><option value="SINGLE">Única</option><option value="RECURRING">Recorrente</option><option value="INSTALLMENT">Parcelada</option></select></Field>}
    ...
    <div className={`${styles.toolbarGroup} ${styles.formWide}`}><button className={styles.button}>{entry ? "Salvar" : settleImmediately ? "Lançar no Caixa" : "Adicionar"}</button><button className={`${styles.button} ${styles.buttonSoft}`} type="button" onClick={onClose}>Cancelar</button>{replacementSettlementId && <button className={`${styles.button} ${styles.buttonDanger}`} type="button" onClick={async () => { if (!window.confirm("Excluir este lançamento do Caixa? O estorno será registrado no histórico.")) return; const saved = await runMutation(setMessage, async () => { await assertResult(await connectedClient().rpc("reverse_financial_settlement", { p_settlement_id: replacementSettlementId, p_amount_cents: entry?.total_cents ?? 0, p_reason: "Exclusão de lançamento manual", p_idempotency_key: `manager:cash-delete:${replacementSettlementId}:${crypto.randomUUID()}` })); }, "Lançamento excluído por estorno."); if (saved) onSaved(); }}>Excluir</button>}</div>
