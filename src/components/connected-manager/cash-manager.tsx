...
function EntryDialog({ entry, defaultKind, settleImmediately = false, organizationId, accounts, suppliers, chartAccounts, costCenters, tags, customers, entryTags, demoMode, onClose, onSaved, setMessage }: Omit<CashManagerProps, "section" | "billingStatus" | "balances" | "entries" | "settlements" | "appointmentActivity" | "mappings" | "appointmentReceivables"> & { entry: FinancialEntryRecord | null; defaultKind: EntryKind; settleImmediately?: boolean; onClose: () => void; onSaved: () => void; setMessage: (value: string) => void }) {
  const router = useRouter();
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    }, entry ? "Lançamento aberto atualizado." : isSeries ? "Série de despesas criada." : settleImmediately ? "Lançamento liquidado e registrado no Caixa." : "Lançamento criado.");
    if (saved) onSaved();
  }
  return <Dialog title={entry ? "Editar lançamento" : "Novo lançamento"} onClose={onClose} wide><form className={styles.form} onSubmit={submit}>
    ...
    <Field label="Banco ou caixa"><select name="account_id" required={settleImmediately} defaultValue={entry?.preferred_financial_account_id ?? ""}><option value="">{settleImmediately ? "Selecione" : "Definir na liquidação"}</option>{accounts.filter((item) => item.active).map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></Field>
    ...
    {settleImmediately && <><Field label="Forma de pagamento"><select name="payment_method" defaultValue="CASH"><option value="CASH">Dinheiro</option><option value="PIX">PIX</option><option value="CARD">Cartão</option><option value="TRANSFER">Transferência</option><option value="OTHER">Outro</option></select></Field><Field label="Observações"><input name="reference" placeholder="PIX, NSU ou comprovante" /></Field></>}
    <div className={`${styles.toolbarGroup} ${styles.formWide}`}><button className={styles.button}>{entry ? "Salvar" : settleImmediately ? "Lançar no Caixa" : "Adicionar"}</button><button className={`${styles.button} ${styles.buttonSoft}`} type="button" onClick={onClose}>Cancelar</button></div>
