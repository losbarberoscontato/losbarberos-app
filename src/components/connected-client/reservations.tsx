...
function appointmentStatusLabel(appointment: CustomerAppointment): string {
  if (appointment.status === "CANCELED") {
    if (appointment.cancellation_outcome === "ON_TIME") return "Cancelado no prazo";
    if (appointment.cancellation_outcome === "AFTER_DEADLINE") return "Cancelado após o prazo";
    return "Cancelado";
  }
  if (appointment.status === "NO_SHOW" && appointment.subscription_session_id) return "Cancelado após o prazo";
...
function ReservationsContent() {
  ...
  const [cancelBeforeDeadline, setCancelBeforeDeadline] = useState(false);
  ...
  async function performCancel() {
    ...
    const { startsAt } = parsePostgresRange(cancelTarget.service_period);
    const beforeDeadline = cancelTarget.cancellation_lead_minutes_snapshot === 0 || Date.now() <= new Date(startsAt).getTime() - cancelTarget.cancellation_lead_minutes_snapshot * 60_000;
    if (!beforeDeadline && !cancelAcknowledged) return;
...
function ReservationCard({ appointment, timezone, barberName, onCancel, onResumePayment, paymentBusy, acceptingBookings }: { appointment: CustomerAppointment; timezone: string; barberName: string; onCancel: () => void; onResumePayment: () => void; paymentBusy: boolean; acceptingBookings: boolean }) {
  ...
  return <article className={styles.reservationCard}><header><span data-status={appointment.status}><Check size={13} /> {appointmentStatusLabel(appointment)}</span><small>{appointment.payment_mode === "SUBSCRIPTION" ? "Plano de assinatura" : appointment.financial.financial_status.replaceAll("_", " ")}</small></header><div className={styles.reservationBody}><div className={styles.dateBox}><strong>{new Intl.DateTimeFormat("pt-BR", { timeZone: timezone, day: "2-digit" }).format(new Date(startsAt))}</strong><span>{new Intl.DateTimeFormat("pt-BR", { timeZone: timezone, month: "short" }).format(new Date(startsAt))}</span></div><div><h2>{appointment.items.map((item) => item.service_name_snapshot).join(" + ") || "Reserva"}</h2><p><Clock3 size={15} /> {formatSlotTime(startsAt, timezone)}–{formatSlotTime(endsAt, timezone)} · {barberName}</p><small>{formatInstant(startsAt, timezone, { dateStyle: "full", timeStyle: undefined })}</small></div><dl>{appointment.payment_mode === "SUBSCRIPTION" ? <div><dt>Pagamento</dt><dd>Incluída no plano</dd></div> : <><div><dt>Total</dt><dd>{formatMoney(appointment.total_cents_snapshot, appointment.currency)}</dd></div><div><dt>Pago líquido</dt><dd>{formatMoney(appointment.financial.net_paid_cents, appointment.currency)}</dd></div><div><dt>Saldo</dt><dd>{formatMoney(appointment.financial.outstanding_cents, appointment.currency)}</dd></div></>}</dl></div>{(canCancel || canResumePayment) && <footer>{canCancel && <button type="button" className={styles.textButton} onClick={onCancel}>Cancelar</button>}{canResumePayment && <button type="button" className={styles.primaryButton} disabled={paymentBusy} onClick={onResumePayment}>{paymentBusy ? "Abrindo…" : "Retomar pagamento"}</button>}</footer>}</article>;
}
