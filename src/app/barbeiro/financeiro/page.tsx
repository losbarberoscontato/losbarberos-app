...
export default async function BarberFinancePage({ searchParams }: { searchParams: Promise<{ barbearia?: string }> }) {
  const params = await searchParams;
  if (!params.barbearia) redirect("/barbeiro");
  const data = await loadBarberCash(params.barbearia);
  if (!data) redirect("/barbeiro");
  return <BarberShell context={data.context}><BarberCash {...data} activeView="overview" /></BarberShell>;
}
